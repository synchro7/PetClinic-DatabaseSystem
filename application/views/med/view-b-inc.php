...
<script>
	$(document).ready(function () {
		...
		$('#edit_med').on('submit', function (event) {
			...
			$.confirm({
				...
				confirm: function () {
					...
					$('#DoB').val(cvdate);
					var sumit_data = $('#edit_med').serializeArray();
					$('#DoB').val(getdate);
					var url = $('#edit_med').attr("action");
					$.post(url, sumit_data).done(function () {
						...
						mes += 'This medicine has been updated.';
						mes += '</div>';
			...
			$.confirm({
				...
				confirm: function () {
					$.ajax({
						...
						window.location.href = '<?php echo base_url(); ?>med';
					}).fail(function () {
						$.alert({
							icon: 'fa fa-exclamation-circle',
