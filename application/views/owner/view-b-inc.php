...
<script>
	$(document).ready(function () {
		...
		$('#edit_owner').on('submit', function (event) {
			...
			$.confirm({
				...
				confirm: function () {
					...
					$('#DoB').val(cvdate);
					var sumit_data = $('#edit_owner').serializeArray();
					$('#DoB').val(getdate);
					var url = $('#edit_owner').attr("action");
					$.post(url, sumit_data).done(function () {
						...
						mes += 'This owner has been updated.';
						mes += '</div>';
			...
			$.confirm({
				...
				confirm: function () {
					$.ajax({
						...
						window.location.href = '<?php echo base_url(); ?>owner';
					}).fail(function () {
						$.alert({
							icon: 'fa fa-exclamation-circle',
